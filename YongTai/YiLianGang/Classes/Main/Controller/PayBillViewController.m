...
-(void)getPropertyBillListData
{
    ...
    NSString *requestUrl = @"http://www.yiliangang.net:8012/Yongtai_community/collectfee/findHouseId";
    NSLog(@"测试：%@",[LoginNewTool sharedLoginNewTool].houseId);
    
    NSDictionary *userInfo = [[NSUserDefaults standardUserDefaults] objectForKey:@"userInfo"];
    NSDictionary *parmDict = @{@"house_id":[userInfo objectForKey:@"houseId"],
                               @"pageNo":@1,
                               @"pageSize":@1000
                               };//请求可接受订单接口
    ...
    [manager GET:requestUrl parameters:parmDict progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        NSArray *listArray = [[responseObject objectForKey:@"msg"] objectForKey:@"list"];
        if (listArray.count==0) {
            [MBProgressHUDUtil showMessage:@"没有账单信息！" toView:self.view];
            return ;
        }

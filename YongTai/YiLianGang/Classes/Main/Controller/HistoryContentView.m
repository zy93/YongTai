...
{
    mContentDic = dic;
    [mContentLab setText:dic[@"content"]];
    [mAddr setText:[NSString stringWithFormat:@"地址：%@",dic[@"address"]]];
    [mTel setText:[NSString stringWithFormat:@"电话：%@",dic[@"tel"]]];
    [mTime setText:[NSString stringWithFormat:@"预约时间：%@",dic[@"ctime"]]];
    mEvaluateView.hidden = YES;
    //待维修状态
    NSString *stateStr = [NSString stringWithFormat:@"%ld",[[dic objectForKey:@"isStatus"] longValue] ];
    
    if ([stateStr isEqualToString:@"0"]) {
        mInfo2ImageBtn.hidden = YES;
        mInfo3ImageBtn.hidden = YES;
        if (strIsEmpty(dic[@"pic1"])) {
            mInfo1ImageBtn.hidden = YES;
        }
        else {
            [self loadImageWithUrl:dic[@"pic1"] button:mInfo1ImageBtn];
        }
        ...
        if(strIsEmpty(dic[@"pic2"])){
            mInfo2ImageBtn.hidden = YES;
        }else
        {
            [self loadImageWithUrl:dic[@"pic2"] button:mInfo2ImageBtn];
        }
    }
    //维修完成状态
        ...
        if (strIsEmpty(dic[@"pic3"])) {
            mInfo3ImageBtn.hidden = YES;
        } else {
            [self loadImageWithUrl:dic[@"pic3"] button:mInfo3ImageBtn];
        }
    }
}
...
-(void)loadImageWithUrl:(NSString *)ur button:(UIButton *)button
{
    ...
    NSString *u = [NSString stringWithFormat:@"%@%@%@",HTTP_Service,@"Yongtai_community",ur];
    NSLog(@"图片地址：%@",u);
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        NSURL *url = [NSURL URLWithString:u];
        dispatch_async(dispatch_get_main_queue(), ^{
             [weak sd_setImageWithURL:url forState:UIControlStateNormal placeholderImage:[UIImage imageNamed:@"Notloaded"]];
        });
    });
}
